...
def main():
    parser = argparse.ArgumentParser(
        prog=__app_name__,
        description="Bilibili anime + Steam news (compliant sources)."
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    sub = parser.add_subparsers(dest="cmd", required=True)

    # bili
    p_b = sub.add_parser("bili", help="Bilibili sources")
    ...
    if args.cmd == "bili" and args.bili_cmd == "timeline":
        ...
        _export(items, args.format, args.outdir, "bili_timeline", "Bilibili PGC Timeline")
        ...
        _export(items, args.format, args.outdir, f"bili_rank_{category}", f"Bilibili Ranking ({category})")
    elif args.cmd == "steam":
        with open("config.yaml", "r", encoding="utf-8") as f:
            cfg = yaml.safe_load(f) or {}
        appids = cfg.get("steam_appids") or []
        items = fetch_steam_news(appids=appids, limit_per_app=args.limit)
        _export(items, args.format, args.outdir, "steam_news", "Steam News")
